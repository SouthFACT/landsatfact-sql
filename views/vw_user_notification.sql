...
SELECT DISTINCT user_aoi.aoi_id,
   ...
   ((((('http://www.landsatfact.com/map/?theme=SE&layers='::text ||
      ...
      vw_last_days_products.product_type::text) ||
      date_part('year'::text, vw_last_days_products.product_date)) ||
      right('0' || date_part('month',  vw_last_days_products.product_date),2)) ||
      right('0' || date_part('day',  vw_last_days_products.product_date),2)) ||
      ',AA&mask=&alphas=1,1&accgp=G02&basemap=Google%20Streets&extent='::text) ||
      replace(replace(replace(st_extent(st_transform(user_aoi.geom, 900913))::text, 'BOX('::text, ''::text), ')'::text, ''::text), ' '::text, ','::text) AS lsf_url
  FROM landsat_quads,
