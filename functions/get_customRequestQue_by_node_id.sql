...
CREATE OR REPLACE FUNCTION public.get_customRequestQue_by_node_id(pass_node_id text)
  RETURNS text AS
$BODY$
  ...
  BEGIN
    ...
    FROM custom_request_dates as cr
     ...
     SELECT date_trunc('minute',avg(timespent)) FROM TEST))::text || ' to complete.' as completetion   from getCRQue where aoi_id = (SELECT aoi_id from user_aoi where node_id = pass_node_id LIMIT 1) ;
...
ALTER FUNCTION public.get_customRequestQue_by_node_id(text)
  OWNER TO root;
GRANT EXECUTE ON FUNCTION public.get_customRequestQue_by_node_id(text) TO public;
GRANT EXECUTE ON FUNCTION public.get_customRequestQue_by_node_id(text) TO root;
GRANT EXECUTE ON FUNCTION public.get_customRequestQue_by_node_id(text) TO dataonly;
GRANT EXECUTE ON FUNCTION public.get_customRequestQue_by_node_id(text) TO readonly;
