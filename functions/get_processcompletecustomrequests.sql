-- Function: public.get_processcompletecustomrequests()

-- DROP FUNCTION public.get_processcompletecustomrequests();

CREATE OR REPLACE FUNCTION public.get_processcompletecustomrequests()
...
$BODY$
  ...
  --returns a postgres table defined bythe type processing complete custom requests
  ...
  WHERE  upper((select aoi_type FROM user_aoi where user_aoi.aoi_id = cr.aoi_id)) = upper($1) AND
         (SELECT max(sub_crd.custom_request_status_id) FROM custom_request_dates sub_crd  WHERE sub_crd.aoi_id = cr.aoi_id) = 3
  GROUP BY cr.aoi_id
  ORDER BY cr.aoi_id,
    (SELECT status
     FROM custom_request_dates sub_crd
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION public.get_processcompletecustomrequests()
  OWNER TO root;
GRANT EXECUTE ON FUNCTION public.get_processcompletecustomrequests() TO public;
GRANT EXECUTE ON FUNCTION public.get_processcompletecustomrequests() TO root;
GRANT EXECUTE ON FUNCTION public.get_processcompletecustomrequests() TO dataonly;
GRANT EXECUTE ON FUNCTION public.get_processcompletecustomrequests() TO readonly;
